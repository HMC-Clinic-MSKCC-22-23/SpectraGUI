...
class SPECTRA(nn.Module):
    ...
    def __init__(self, X, labels, adj_matrix, L, weights=None, lam=10e-4, delta=0.1, kappa=0.00001, rho=0.00001,
                 use_cell_types=True):
        ...
        if use_cell_types:
            ...
            self.adj_matrix = {cell_type: torch.Tensor(adj_matrix[cell_type]) - torch.Tensor(
                np.diag(np.diag(adj_matrix[cell_type]))) if len(adj_matrix[cell_type]) > 0 else [] for cell_type in
                               adj_matrix.keys()}
            # for convenience store 1 - adjacency matrix elements [except on diagonal, where we store 0]
            adj_matrix_1m = {cell_type: 1.0 - adj_matrix[cell_type] if len(adj_matrix[cell_type]) > 0 else [] for
                             cell_type in adj_matrix.keys()}  # one adj_matrix per cell type
            self.adj_matrix_1m = {
                cell_type: torch.Tensor(adj_matrix_1m[cell_type] - np.diag(np.diag(adj_matrix_1m[cell_type]))) if len(
                    adj_matrix_1m[cell_type]) > 0 else [] for cell_type in
                adj_matrix_1m.keys()}  # one adj_matrix per cell type

            # if weights are provided, convert these to tensors, else set weights = to adjacency matrices
            if weights:
                self.weights = {cell_type: torch.Tensor(weights[cell_type]) - torch.Tensor(
                    np.diag(np.diag(weights[cell_type]))) if len(weights[cell_type]) > 0 else [] for cell_type in
                                weights.keys()}
            else:
                self.weights = self.adj_matrix

            self.cell_types = np.unique(
                labels)  # cell types are the unique labels, again require knowledge of labels at initialization but do not store them
    ...
    def loss(self, X, labels):
        ...
        for cell_type in self.cell_types:
            ...
            if len(adj_matrix) > 0:
                ...
                term2 = -1.0 * (
                    torch.xlogy(adj_matrix * weights, (1.0 - rho) * (1.0 - kappa) * mat + (1.0 - rho) * kappa)).sum()
        ...
        if len(adj_matrix) > 0:
            ...
            term2 = -1.0 * (torch.xlogy(adj_matrix * weights, (1.0 - rho_global) * (1.0 - kappa_global) * mat + (
                        1.0 - rho_global) * kappa_global)).sum()
            term3 = -1.0 * (
                torch.xlogy(adj_matrix_1m, (1.0 - kappa_global) * (1.0 - rho_global) * (1.0 - mat) + rho_global)).sum()
...
class LitModel(pl.LightningModule):
    ...
    # Initializes lightning object
    def __init__(self,encoder):
        super().__init__()
        self.encoder = encoder


    # How do we mesh the loss function, provided by SPECTRA, with this lightning step?
    ...
    # How do we mesh this with the lr_scheduler given in SPECTRA code?
...
class SPECTRA_Model:
    ...
    def __init__(self, X, labels, L, vocab=None, gs_dict=None, use_weights=False, adj_matrix=None, weights=None,
                 lam=0.1, delta=0.1, kappa=None, rho=None, use_cell_types=True):
        self.L = L
        self.lam = lam
        self.delta = delta
        self.kappa = kappa
        ...
        self.internal_model = SPECTRA(X=X, labels=labels, adj_matrix=adj_matrix, L=L, weights=weights, lam=lam,
                                      delta=delta, kappa=kappa, rho=rho, use_cell_types=use_cell_types)
    ...
    def __store_parameters(self, labels):
        ...
        for i in range(len(f)):
            ct = f[i]
            scaled = factors[i, :] * (model.gene_scaling[ct].exp().detach() / (
                        1.0 + model.gene_scaling[ct].exp().detach()) + model.delta).numpy()
        ...
        self.gene_scalings = {
            ct: model.gene_scaling[ct].exp().detach().numpy() / (1.0 + model.gene_scaling[ct].exp().detach().numpy())
            for ct in model.gene_scaling.keys()}
        self.rho = {ct: model.rho[ct].exp().detach().numpy() / (1.0 + model.rho[ct].exp().detach().numpy()) for ct in
                    model.rho.keys()}
        self.kappa = {ct: model.kappa[ct].exp().detach().numpy() / (1.0 + model.kappa[ct].exp().detach().numpy()) for ct
                      in model.kappa.keys()}
    ...
    def __store_parameters_no_celltypes(self):
        ...
        scaled = factors * (model.gene_scaling.exp().detach() / (
                    1.0 + model.gene_scaling.exp().detach()) + model.delta).numpy().reshape(1, -1)
        new_factors = scaled / (scaled.sum(axis=0, keepdims=True) + 1.0)
...
class SPECTRA_EM:
    ...
    def E_step(self):
        ...
        log_term3 = contract('ij,kl->ijkl', self.A1m,
    ...
    def update_theta_newton(self):
        ...
        evidence = self.phi.sum(axis=0) + self.lam * (
                    self.phi_tilde.sum(axis=(1, 3)) + self.phi_tilde.sum(axis=(0, 2)))  # P x K
        inverse_hessian = -1.0 * (self.theta ** 2) / (evidence + self.EPS)  # analytic second derivative
        inverse_hessian[~np.isfinite(inverse_hessian)] = 0  # control instability
        gh = -1.0 * (self.theta - (self.theta ** 2) / evidence * (self.g + self.delta).reshape(-1, 1) * (
        ...
        self.theta = self.theta / self.theta.sum(axis=1,
                                                 keepdims=True)  # ensure sum to 1 constraint. - without numerical error does nothing
        return
    ...
    def update_B(self):
        ...
        self.B = ((self.rho / (1.0 - self.rho) + 1.0 - self.kappa) * evidence_num - self.kappa * evidence_denom) / (
...
def est_spectra(adata, gene_set_dictionary, L=None, use_highly_variable=True, cell_type_key=None, use_weights=True,
    ...
    spectra = SPECTRA_Model(X=X, labels=labels, L=L, vocab=vocab, gs_dict=gene_set_dictionary, use_weights=use_weights,
                            lam=lam, delta=delta, kappa=kappa, rho=rho, use_cell_types=use_cell_types)
...
def load_from_pickle(fp, adata, gs_dict, cell_type_key):
    model = SPECTRA_Model(X=adata[:, adata.var["spectra_vocab"]].X, labels=np.array(adata.obs[cell_type_key]),
